...
const PersistedComposerThreadDraftState = Schema.Struct({
  ...
  // `codex` / `claudeAgent` / `kiro` / ... entries. Every prior `ProviderDriverKind`
...
export interface ComposerThreadDraftState {
   ...
   * `kiro` / `opencode`) also satisfies the `ProviderInstanceId` slug pattern, so
...
function normalizeProviderModelOptions(
  ...
  const result: ProviderOptionSelectionsByProvider = {};
  for (const providerKey of ["codex", "claudeAgent", "cursor", "kiro", "opencode"] as const) {
...
function legacyToModelSelectionByProvider(
  ...
  if (modelOptions) {
    for (const provider of ["codex", "claudeAgent", "cursor", "kiro", "opencode"] as const) {
